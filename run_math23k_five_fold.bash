...
for (( d=0; d<${#var_update_modes[@]}; d++  )) do
    ...
    for (( e=0; e<${#bert_model_names[@]}; e++  )) do
        ...
        for (( f=0; f<${#folds[@]}; f++  )) do
          fold_num=${folds[$f]}
          dev=${cuda_devices[$e]}
          model_folder=math23k_${bert_model_name}_${var_update_mode}_${fold_num}
          TOKENIZERS_PARALLELISM=false \
          python3 universal_main.py --device=cuda:${dev} --model_folder=${model_folder} --mode=train --height=10 --train_max_height=14 --num_epochs=1000 --consider_multiple_m0=${consider_multiple_m0} \
                          --train_file=data/math23k_five_fold/train_${fold_num}.json --add_replacement=${add_replacement} --train_num=-1 --dev_num=-1 \
                          --dev_file=data/math23k_five_fold/test_${fold_num}.json --use_constant=${use_constant} --add_new_token=${add_new_token} \
