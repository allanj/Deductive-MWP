...
def initialize_param(cls, config, constant_num, height, var_update_mode, temperature, use_contrastive):
...
def pretrain_rep(cls,
    ...
    for i in range(max_height):
        ...
        if i == 0:
            ...
            ## batch_size, num_combinations/num_m0, num_labels, hidden_size
            m0_label_rep = torch.stack([layer(m0_hidden_states) for layer in linear_modules], dim=2)
            if cls.use_contrastive and not is_eval:
                m0_label_rep_drop = torch.stack([layer(m0_hidden_states) for layer in linear_modules], dim=2)
            ...
            if labels is not None and not is_eval:
                ...
                if cls.use_contrastive:
                    ...
                    cs_loss_funct = nn.CrossEntropyLoss()
                    cs_loss = cs_loss_funct(sim_logits, m0_gold_labels[:, 2])
                    loss = loss + cs_loss
            ...
            mi_label_rep = torch.stack([layer(mi_hidden_states) for layer in linear_modules], dim=2)
            if cls.use_contrastive and not is_eval:
                mi_label_rep_drop = torch.stack([layer(mi_hidden_states) for layer in linear_modules], dim=2)
            mi_logits = cls.label_rep2label(mi_label_rep).expand(batch_size, num_combinations, cls.num_labels, 2)
            mi_logits = mi_logits + batched_combination_mask.unsqueeze(-1).unsqueeze(-1).expand(batch_size, num_combinations, cls.num_labels,
                                                                                                2).float().log()
            ...
            if labels is not None and not is_eval:
                ...
                if cls.use_contrastive:
                    ...
                    sim_logits = cos(gold_label_rep, gold_comb_rep) / cls.temp  # batch_size x label_size
                    cs_loss_funct = nn.CrossEntropyLoss()
                    cs_loss = cs_loss_funct(sim_logits, mi_gold_labels[:, 2])
                    loss = loss + cs_loss
            else:
                best_mi_label_rep = mi_label_rep[b_idxs, best_comb, best_label]  # batch_size x hidden_size
                best_mi_scores = mi_logits[b_idxs, best_comb, best_label][:, 0]
