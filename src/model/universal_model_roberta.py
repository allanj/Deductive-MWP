    ...
    def beam_search(self,
        ...
        for i in range(max_height):
            ...
            if i == 0:
                ...
                top_k_values, top_k_index = m0_combined_logits.view(batch_size, -1).topk(k=beam_size, dim=-1) ## batch_size x beam_size
                ## Note: we want to get.
                ## Note: current_best_beam: (batch_size, beam_size, 4 ](left_idx, right_idx, label_id, stop_id)

                best_a = torch.floor(top_k_index / (self.num_labels * 2)).long() ## comb_id. batch_size x beamsize
                best_b= torch.floor((top_k_index - self.num_labels * 2 *best_a)/2).long() ## label_id. batch_size x beam_size
                best_c = torch.remainder(top_k_index, 2).long() #2: stop id, batch_size x 2

                batch_comb = combination.unsqueeze(0).expand(batch_size, num_combinations, 2)
                best_comb = batch_comb[b_idxs.repeat_interleave(beam_size), best_a.view(-1)].view(batch_size, beam_size, 2)
