...
class UniversalModel_Roberta(RobertaPreTrainedModel):
    ...
    def __init__(self, config: RobertaConfig,
        ...
        if self.constant_num > 0:
            ...
            print("[MODLE INFO] You are using contextualzied multi head attention for constant representation!!!!!!!")
            self.multihead_attention = nn.MultiheadAttention(embed_dim=config.hidden_size, num_heads=6, batch_first=True)
            if self.use_constant_scorer:
                self.constant_scorer = nn.Sequential(
                    nn.Linear(config.hidden_size, config.hidden_size),
                    nn.ReLU(),
                    nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps),
                    nn.Dropout(config.hidden_dropout_prob),
    ...
    def forward(self,
        ...
        if self.constant_num > 0:
            ...
            updatd_variable_index_mask = torch.cat([const_idx_mask, variable_index_mask], dim = 1)

            ## added for multihead_attention
            const_eye = torch.eye(self.constant_num, device=variable_indexs_start.device).long()
            temp_1 = torch.ones(max_num_variable, self.constant_num, device=variable_indexs_start.device).long()
            temp_2 = torch.ones(self.constant_num + max_num_variable, max_num_variable, device=variable_indexs_start.device)
            attn_temp_mask = torch.cat([torch.cat([const_eye, temp_1], dim=0), temp_2], dim=1).unsqueeze(0).expand(
                batch_size * self.multihead_attention.num_heads, self.constant_num + max_num_variable,
                self.constant_num + max_num_variable)
            temp_key_mask = updatd_variable_index_mask.repeat_interleave(self.multihead_attention.num_heads, dim=0).unsqueeze(1).expand(
                batch_size * self.multihead_attention.num_heads, self.constant_num + max_num_variable, self.constant_num + max_num_variable)  ##
            attn_temp_mask = (1 - attn_temp_mask * temp_key_mask).bool()
            updated_all_states, _ = self.multihead_attention(var_hidden_states, var_hidden_states, var_hidden_states,
                                                             key_padding_mask=(1 - updatd_variable_index_mask).bool(),
                                                             attn_mask=attn_temp_mask)
            var_hidden_states = torch.cat([updated_all_states[:, :self.constant_num, :], var_hidden_states[:, self.constant_num:, :]], dim=1)

            max_num_variable = max_num_variable + self.constant_num
            variable_index_mask = updatd_variable_index_mask
        ...
        for i in range(max_height):
            ...
            if i == 0:
                ...
                if not self.consider_multiple_m0:
                    ...
                    var_hidden_states = self.variable_gru(gru_inputs, init_h).view(batch_size, max_num_variable + i - 1,
                                                                                   hidden_size)
...
def test_case_batch_two():
    model = UniversalModel_Roberta.from_pretrained('hfl/chinese-roberta-wwm-ext', num_labels=6, constant_num=2)
